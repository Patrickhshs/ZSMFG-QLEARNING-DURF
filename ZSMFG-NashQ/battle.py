...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, default='mfq', help='choose an algorithm from the preset', required=False)
    parser.add_argument('--oppo', type=str, default='mfq', help='indicate the opponent model')
    ...
    models = [spawn_ai(args.algo, device, env, handles[0], args.algo + '-me', args.max_steps), spawn_ai(args.oppo, device, env, handles[1], args.oppo + '-opponent', args.max_steps)]

    models[0].load(main_model_dir, step=args.idx[0])
    models[1].load(oppo_model_dir, step=args.idx[1])

    runner = tools.Runner(device, env, handles, args.map_size, args.max_steps, models, battle, render_every=0)

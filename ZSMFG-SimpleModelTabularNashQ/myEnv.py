...
from gametheory.normal_form import Player, NormalFormGame
import random
...
class my1dGridEnv(object):
    ...
    def find_nash_gambit(self,A,B):
        game = NormalFormGame((Player(A), Player(B)))

        equilibria = game.solve()
        for i in equilibria:
            print(i)
        pi1 = equilibria[random.randrange(len(equilibria))].strategy(0)
        pi2 = equilibria[random.randrange(len(equilibria))].strategy(1)
        return pi1,pi2
